...
public class ImageDetectServiceImpl implements ImageDetectService {
     ...
     * @param src 图片
     */
    private void saveMarkedImage(Mat src, String filePath) {
        // 新的图片文件名称
        String newFilePath = this.transDetectImage(filePath);

        // 图片写到磁盘上
        imwrite(newFilePath, src);
    }

    @Override
    public String transDetectImage(String imageName) {
        int index = imageName.lastIndexOf(".");
        String fileName = imageName.substring(0, index);
        String fileExtension = imageName.substring(index);
        return fileName + detectImageSuffix + fileExtension;
    }
}
